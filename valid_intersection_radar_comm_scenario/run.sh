...
OUT_ROOT="${OUT_ROOT:-$HOME/NEWWAY_runs/$DATE_TAG/valid_intersection_radar_comm_scenario}"
SUMO_GUI="${SUMO_GUI:-1}"
...
SUMO_CONFIG="${SUMO_CONFIG:-$ROOT/valid_intersection_radar_comm_scenario/sumo/map_intersection_radar_link.sumo.cfg}"
...
if [[ "$USE_SIONNA" != "1" ]]; then
  echo "valid_intersection_radar_comm_scenario is Sionna-only. Set USE_SIONNA=1." >&2
  exit 2
fi
...
if [[ "$CHECK_SIONNA_LISTENER" == "1" ]]; then
  if ! wait_for_sionna_listener; then
    ...
    echo "  valid_intersection_radar_comm_scenario/start_sionna_server.sh" >&2
    exit 3
  fi
fi
...
"$PY_BIN" "$ROOT/valid_intersection_radar_comm_scenario/summarize_runs.py" \
  --runs-root "$OUT_ROOT" \
  --modes "${SUMMARY_MODES[@]}" \
