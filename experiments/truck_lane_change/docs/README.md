...
```bash
truck_lane_change/run.sh
```

Live GUI:

```bash
SUMO_GUI=1 truck_lane_change/run.sh
```

Примечание для Sionna: `truck_lane_change/run.sh` по умолчанию запускает сценарий с `USE_SIONNA=1`, поэтому Sionna server должен быть доступен на `SIONNA_SERVER_IP` (по умолчанию `127.0.0.1`).

Headless:

```bash
SUMO_GUI=0 truck_lane_change/run.sh
```

Fallback без Sionna:

```bash
USE_SIONNA=0 truck_lane_change/run.sh
```

Куда пишутся результаты по умолчанию:

- `$HOME/NEWWAY_runs/<YYYY-MM-DD>/truck_lane_change`
...
- `CRASH_MODE_ENABLE=auto` (по умолчанию в `truck_lane_change/run.sh`)
...
- `artifacts/truck_lane_change_story/*` — дипломные графики «под капотом»

## Подкапотные графики (автоматически)

`truck_lane_change/run.sh` вызывает:

- `runs/build_truck_lane_change_story_plots.py`
- `runs/build_truck_lane_change_intuitive_plots.py`
...
Отдельно формируются графики в `artifacts/truck_lane_change_intuitive/`:
...
Готовый расширенный текст сценария: `truck_lane_change/VKR_SCENARIO_TEXT.md`.
