...
if [[ -x "$ROOT/tools/plots/build_intersection_scenario_summary.py" ]] || \
   [[ -f "$ROOT/tools/plots/build_intersection_scenario_summary.py" ]]; then
...
if [[ "$VISUALIZE" == "1" ]]; then
  ...
  "$PY_BIN" "$ROOT/experiments/intersection_v2x_awareness/tools/visualize_collision_causality.py" \
    --run-dir "$OUT_DIR" \
    --focus-vehicle "${ROLE_OBSTRUCTED_ID}" \
    --tx-id "${EMERGENCY_STATION_ID}" \
