...
OUT_ROOT="${OUT_ROOT:-$HOME/NEWWAY_runs/$DATE_TAG/cpm_perception}"
SUMO_GUI="${SUMO_GUI:-0}"
...
if [[ "$USE_SIONNA" != "1" ]]; then
  echo "cpm_perception is Sionna-only. Set USE_SIONNA=1." >&2
  exit 2
fi
...
if [[ "$CHECK_SIONNA_LISTENER" == "1" ]]; then
  if ! ss -lunH 2>/dev/null | awk '{print $4}' | grep -Fq ":${SIONNA_PORT}"; then
    ...
    echo "  experiments/cpm_perception/scripts/start_sionna_server.sh" >&2
    exit 3
  fi
fi
...
"$PY_BIN" "$ROOT/experiments/cpm_perception/scripts/summarize_runs.py" \
  --runs-root "$OUT_ROOT" \
  --modes "${SUMMARY_MODES[@]}" \
