...
OUT_BASE="${OUT_BASE:-$ROOT/runs/$(date +%F)/cam-sionna-backend-compare-$(date +%H%M%S)}"
...
OUT_DIR="$OUT_NO" NS3_DIR="$NS3_DIR" RUN_ARGS="--sumo-gui=0 --sim-time=$SIM_TIME --sionna=0" \
  "$ROOT/experiments/operational/v2v-cam-exchange-sionna-nrv2x/run.sh"
...
if [[ "$EXPORT_RESULTS" == "1" ]]; then
  ...
  if ! "$SIONNA_PY" "$ROOT/tools/analysis/export_results_bundle.py" "${export_args[@]}"; then
