...
OUT_BASE="${OUT_BASE:-$ROOT/runs/$(date +%F)/eva-short-sionna-gpu-only-tx23-$(date +%H%M%S)}"
...
SIONNA_PORT="$SIONNA_PORT" \
  bash "$ROOT/experiments/operational/v2v-emergencyVehicleAlert-nrv2x/run_sionna_incident_sweep.sh"
