...
NS3_DIR="${NS3_DIR:-}"
OUT_DIR="${OUT_DIR:-$ROOT/runs/$(date +%F)}"
JOBS="${JOBS:-8}"
...
EXPORT_ROOT="${EXPORT_ROOT:-$ROOT/runs/chatgpt_exports}"
...
if [[ -f "$NETSTATE_FILE" ]]; then
  "$PY_BIN" "$ROOT/tools/analysis/analyze_netstate_collision_risk.py" \
...
if [[ "$EVENT_TIMELINE" == "1" ]]; then
  if ! "$PY_BIN" "$ROOT/tools/plots/build_drop_decision_timeline.py" \
    --run-dir "$OUT_DIR" \
...
if [[ "$COLLISION_CAUSALITY" == "1" ]] && [[ -f "$COLLISION_OUTPUT_FILE" ]] && [[ -f "$EVENT_TIMELINE_DIR/event_timeline.csv" ]]; then
  ...
  if ! "$PY_BIN" "$ROOT/tools/plots/build_collision_causality_report.py" "${causality_args[@]}"; then
    echo "Warning: collision causality report generation failed for v2v-emergencyVehicleAlert-nrv2x"
  fi
fi

if [[ "$PLOT" == "1" ]]; then
  if ! "$PY_BIN" "$ROOT/tools/plots/make_plots.py" \
    --run-dir "$OUT_DIR" \
...
if [[ "$EXPORT_RESULTS" == "1" ]]; then
  ...
  if ! "$PY_BIN" "$ROOT/tools/analysis/export_results_bundle.py" "${export_args[@]}"; then
