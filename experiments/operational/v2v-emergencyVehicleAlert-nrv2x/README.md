...
```bash
./.venv/bin/python runs/build_drop_decision_timeline.py \
...
- `EXPORT_ROOT` — корень export-папки (по умолчанию `runs/chatgpt_exports`)
...
- Лог: `runs/<date>/.../v2v-emergencyVehicleAlert-nrv2x.log`
- CSV по каждому ТС:
  - `*-CAM.csv`
  - `*-MSG.csv`
  - `*-CTRL.csv`
