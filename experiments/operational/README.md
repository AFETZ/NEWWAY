...
- `EXPORT_RESULTS=0|1` — дублировать результаты в `runs/chatgpt_exports/` (по умолчанию `1`)
...
- Логи и артефакты складываются в `runs/<YYYY-MM-DD>/`.
- Базы SQLite (`.db`) и CSV-артефакты попадают в `runs/<YYYY-MM-DD>/artifacts/`.
- После каждого `run.sh` автоматически строятся графики в `runs/<YYYY-MM-DD>/figures/`.
...
- Полный аудит логов по всем прогонам:
  `./.venv/bin/python runs/analyze_all_logs.py --root runs --out-dir runs`
